...
| Data encryption at rest | Sensitive values (PAN, bank account numbers) must be encrypted at field/application layer; disk-level encryption alone is insufficient (PCI DSS 3.4.1, 3.5.1.2) | Field-level AES-256-GCM envelope encryption applied at application layer before writes to Spanner vault tables. Each sensitive value is stored as `encrypted_value` regardless of data type. Storage-layer encryption is defense-in-depth only. | Encryption design doc, integration test evidence, schema review, SAST confirmation of no plaintext sensitive value writes |
| Value fingerprint integrity | Keyed hash required for indexed sensitive value lookups; unkeyed SHA-256 prohibited (PCI DSS 3.5.1.1) | HMAC-SHA-256 computed using Cloud KMS-managed Fingerprint Key (PFK); `value_fingerprint` is generated for each sensitive value (PAN or bank account number); PFK is institution-scoped (one per institution), HSM-backed, and distinct from PEK | HMAC implementation test evidence, KMS PFK key metadata, code review confirming no unkeyed hash path |
...
| Log redaction | Prevent PAN/CVV/bank account numbers in logs | Structured logging middleware with denylist redaction and payload validation | Log policy config, redaction unit tests, sample sanitized logs |
| Automated sensitive data discovery | Detective scanning for out-of-vault presence of PAN and bank account numbers (PCI DSS 4.0 Req 12.5.2) | Automated discovery tool scanning Cloud Logging buckets, BigQuery audit tables, error reporting, Spanner exports, and CI/CD artifacts at minimum annually and after significant infrastructure changes | Discovery scan reports, remediation records, scan schedule attestation |
...
| Incident response | Detect and respond to security events | Alerting for authz failures, unusual detokenize spikes, key access anomalies, and sensitive data discovery findings | Incident runbook, drill records, postmortem templates |
...
- Annual automated sensitive data discovery scan covering PAN and bank account numbers (plus ad-hoc after infrastructure changes).
